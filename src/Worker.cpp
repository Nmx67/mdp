...
void Worker::exec(const std::string &address, const std::string &serviceName, WorkerTask::Transform transform)
{
   while(true) {
      monitor_.reset();
...
{
   TRACE(TraceLevel::Debug, " started servicing ", serviceName);

   for(uint64_t cntr = 0; /* forever */; ++cntr) {
      TRACE(TraceLevel::Debug, " [", cntr, "] polling");
