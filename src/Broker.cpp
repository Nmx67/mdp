...
void Broker::exec(const std::string &address)
{
   while(true) {
      ...
      try {
         ...
         TRACE(TraceLevel::Error, this, " ", e.what(), " restarting");
      } catch(...) {
         TRACE(TraceLevel::Error, this, " unsupported exception, exiting");
         return;
      }
   }
}
...
void Broker::onMessage(MessageHandle handle)
{
   ...
   try {
      ...
      TRACE(TraceLevel::Warning, this, ' ', e.what());
...
void Broker::dispatch(Tagged<Tag::ClientRequest> tagged)
{
   ...
   if(4 > tagged.handle->parts()) {
      dispatch(
            Tagged<Tag::ClientReply>(makeFailureClientRep(clientIdentity, "", Signature::serviceUndefined))
      );
      return;
   }
   ...
   if(!workerPool_.valid(serviceName)) {
      ...
      dispatch(
         Tagged<Tag::ClientReply>(makeFailureClientRep(clientIdentity, serviceName, Signature::serviceUnsupported))
      );
      return;
   }
   ...
   if(nullptr == worker) {
      dispatch(
         Tagged<Tag::ClientReply>(makeFailureClientRep(clientIdentity, serviceName, Signature::serviceBusy))
      );
      return;
   }
...
void Broker::dispatch(Tagged<Tag::WorkerReply> tagged)
{
   ...
   auto reply = MDP::Broker::makeSucessClientRep(clientIdentity, workerIterator->serviceName_);
...
void Broker::dispatch(Tagged<Tag::WorkerDisconnect> tagged)
{
   ...
   if(brokerTasks_.valid(workerIdentity)) {
      ...
      dispatch(
         Tagged<Tag::ClientReply>(MDP::Broker::makeFailureClientRep(taskInfo.clientIdentity_, workerIterator->serviceName_, MDP::Broker::Signature::serviceFailure))
      );
      brokerTasks_.remove(workerIdentity);
   }
      ...
      if(brokerTasks_.valid(identity)) {
         ...
         dispatch(
            Tagged<Tag::ClientReply>(
               MDP::Broker::makeFailureClientRep(taskInfo.clientIdentity_, iterator->serviceName_, MDP::Broker::Signature::serviceFailure)
            )
         );
